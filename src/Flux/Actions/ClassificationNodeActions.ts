...
import { ClassificationNodeKey, ClassificationNodeStore } from "../Stores/ClassificationNodeStore";
import { ClassificationNodeActionsHub } from "./ActionsHub";

import { TreeStructureGroup, WorkItemClassificationNode } from "TFS/WorkItemTracking/Contracts";
import * as WitClient from "TFS/WorkItemTracking/RestClient";

export module ClassificationNodeActions {
    var classificationNodeStore: ClassificationNodeStore = StoreFactory.getInstance<ClassificationNodeStore>(ClassificationNodeStore);
    ...
    export async function initializeIterationPaths() {
        if (classificationNodeStore.isLoaded(ClassificationNodeKey.Iteration)) {
            ...
            try {
                const rootNode: WorkItemClassificationNode = await WitClient.getClient().getClassificationNode(VSS.getWebContext().project.id, TreeStructureGroup.Iterations, null, 5);
